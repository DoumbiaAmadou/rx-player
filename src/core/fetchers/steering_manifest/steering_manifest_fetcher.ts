...
export default class SteeringManifestFetcher {
  ...
  private _defaultContentSteeringManifestUrl : string;
  private _pipelines : ITransportSteeringManifestPipeline;

  /**
   * Construct a new SteeringManifestFetcher.
   * @param {string} url - Default Steering Manifest url, will be used when no
   * URL is provided to the `fetch` function.
  ...
  constructor(
    ...
    this._defaultContentSteeringManifestUrl = url;
  ...
  public async fetch(
    ...
    const requestUrl = url ?? this._defaultContentSteeringManifestUrl;
    const backoffSettings = this._getBackoffSetting((err) => {
      onRetry(errorSelector(err));
    });
    const callLoader = () => pipelines.loadSteeringManifest(requestUrl, cancelSignal);
    const response = await tryRequestPromiseWithBackoff(callLoader,
                                                        backoffSettings,
                                                        cancelSignal);
